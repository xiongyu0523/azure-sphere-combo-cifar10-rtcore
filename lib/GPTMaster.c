...
static GPTMaster context[MT3620_UNIT_GPT_COUNT] = {0};


static inline int32_t GPTMaster__GetCtrlValue(
    GPTMaster *handle,
    bool      *enabled,
    GPT_Mode  *mode,
    bool      *speed,
...
static inline int32_t GPTMaster__SetCtrlValue(
    GPTMaster *handle,
    bool      *enabled,
    GPT_Mode  *mode,
    bool      *speed,
...
bool GPTMaster_IsEnabled(GPTMaster *handle)
{
    if (!handle) {
        return false;
    }
    bool enabled;
    if (GPTMaster__GetCtrlValue(
...
static inline int32_t GPTMaster__SetEnabled(
    GPTMaster *handle,
    bool       enabled)
{
    if (!handle) {
    ...
    int32_t error = GPTMaster__SetCtrlValue(handle, &enabled, NULL, NULL, NULL);

    return error;
}

static inline int32_t GPTMaster__ToggleRestart(
    GPTMaster *handle,
    bool       restart)
{
    if (!handle) {
    ...
    int32_t error = GPTMaster__SetCtrlValue(handle, NULL, NULL, NULL, &restart);
    return error;
}

static inline int32_t getAvailableSpeeds(
    const uint32_t id, uint32_t *lowSpeed, uint32_t *highSpeed)
{
    switch (id) {
    case MT3620_UNIT_GPT0:
    case MT3620_UNIT_GPT1:
    ...
    case MT3620_UNIT_GPT4:
        *lowSpeed  = MT3620_GPT_4_LOW_SPEED;
        *highSpeed = MT3620_GPT_4_HIGH_SPEED;
        break;
...
static inline int32_t GPTMaster__GetSpeed3(GPTMaster *handle, uint32_t *speedHz)
{
    if (!handle || !speedHz) {
        return ERROR_PARAMETER;
    }
...
int32_t GPTMaster_GetSpeed(GPTMaster* handle, uint32_t *speedHz)
{
    if (!handle || !speedHz) {
        return ERROR_PARAMETER;
    }
    ...
    if (handle->id == MT3620_UNIT_GPT3) {
        return GPTMaster__GetSpeed3(handle, speedHz);
    }

    // Determine available speeds
    uint32_t lowSpeed, highSpeed;
    getAvailableSpeeds(handle->id, &lowSpeed, &highSpeed);

    bool speed_reg;
    int32_t error;
    if ((error = GPTMaster__GetCtrlValue(
...
static inline int32_t GPTMaster__SetSpeed3(
    GPTMaster *handle,
    uint32_t   speedHz)
{
    if (!handle) {
...
static inline int32_t GPTMaster__SetSpeed(
    GPTMaster *handle,
    uint32_t   speedHz)
{
    if (!handle) {
    ...
    if (GPTMaster_IsEnabled(handle)) {
        return ERROR_BUSY;
    }

    if (handle->id == MT3620_UNIT_GPT3) {
        return GPTMaster__SetSpeed3(handle, speedHz);
    }
    ...
    int32_t error = GPTMaster__SetCtrlValue(
        handle, NULL, NULL, &registerValue, NULL);
    return error;
}

int32_t GPTMaster_GetMode(
    GPTMaster *handle,
    GPT_Mode  *mode)
{
    if (!handle || !mode) {
...
static inline int32_t GPTMaster__SetMode(
    GPTMaster *handle,
    GPT_Mode   mode)
{
    if (!handle) {
    ...
    return GPTMaster__SetCtrlValue(handle, NULL, &mode, NULL, NULL);
}

uint32_t GPTMaster_GetCount(GPTMaster *handle)
{
...
uint32_t GPTMaster_GetRunningTime(GPTMaster *handle, GPT_Units units)
{
    if (!handle || (handle->id >= MAX_TIMER_INDEX)) {
        return 0;
    }
    ...
    uint32_t speed;
    if ((GPTMaster_GetSpeed(handle, &speed) != ERROR_NONE) || (speed == 0)) {
        return 0;
    }

    // Get relative count (some timers count backwards, others from non-zero start)
    uint32_t count = GPTMaster_GetCount(handle);
    uint32_t init_count;
    switch (handle->id) {
    case MT3620_UNIT_GPT0:
...
int32_t GPTMaster_GetNumCycles(GPTMaster* handle, uint32_t* numCycles)
{
    if (!handle || !numCycles) {
        return ERROR_PARAMETER;
    }
...
static inline void toggleGPTInterrupts(uint32_t id, bool enable)
{
...
GPTMaster* GPTMaster_Open(int32_t id)
{
    int32_t index = id - MT3620_UNIT_GPT0;
    ...
    if (GPTMaster_IsEnabled(&(context[index]))) {
        return NULL;
    }
    ...
    toggleGPTInterrupts(id, true);

    return &context[index];
}

void GPTMaster_Close(GPTMaster *handle)
{
    ...
    if (GPTMaster_IsEnabled(handle)) {
        GPTMaster_Stop(handle);
    }

    // Disable interrupts
    toggleGPTInterrupts(handle->id, false);

    handle->open = false;
}


int32_t GPTMaster_Stop(GPTMaster *handle)
{
    if (!handle || !handle->open) {
        return ERROR_PARAMETER;
    }
    ...
    // Disable timer
    GPTMaster__SetEnabled(handle, false);
...
int32_t GPTMaster_Pause(GPTMaster *handle)
{
    if (!handle) {
        return ERROR_PARAMETER;
    }
    ...
    handle->initCnt = GPTMaster_GetCount(handle);

    int32_t error;
    if ((error = GPTMaster__SetEnabled(handle, false)) != ERROR_NONE) {
        return error;
    }
...
int32_t GPTMaster_Resume(GPTMaster *handle)
{
    if (!handle) {
        return ERROR_PARAMETER;
    }
    ...
    int32_t error;
    if ((error = GPTMaster__SetEnabled(handle, true)) != ERROR_NONE) {
        return error;
    }
    handle->paused = false;
    return ERROR_NONE;
    ...
    GPT_Units units)
{
    ...
    return (timeout * speed) / units;
}

int32_t GPTMaster_StartTimeout(
    GPTMaster *handle,
    uint32_t   speedHz,
    GPT_Mode   mode,
    uint32_t   timeout,
    ...
    void       (*callback)(GPTMaster *))
{
    ...
    int32_t error;
    if ((error = GPTMaster__SetSpeed(handle, speedHz)) != ERROR_NONE) {
        return error;
    }
    if ((error = GPTMaster__SetMode(handle, mode)) != ERROR_NONE) {
        return error;
    }

    // Fail silently if not supported
    GPTMaster__ToggleRestart(handle, true);

    uint32_t actualSpeed;
    if ((error = GPTMaster_GetSpeed(handle, &actualSpeed)) != ERROR_NONE) {
        return error;
    }
    ...
    case MT3620_UNIT_GPT0:
        ...
        // TODO: do we need to call BlockIrqs() here?
    ...
    case MT3620_UNIT_GPT1:
        ...
        // Set initial count
        mt3620_gpt->gpt1_icnt = nCount;
        break;

    case MT3620_UNIT_GPT3:
        mt3620_gpt->gpt3_init   = 0;
        mt3620_gpt->gpt3_expire = nCount;
        MT3620_GPT_FIELD_WRITE(gpt3_ctrl, gpt3_iclr, true);
        break;

    case MT3620_UNIT_GPT2:
    case MT3620_UNIT_GPT4:
        return ERROR_GPT_TIMEOUT_NOT_SUPPORTED;
    ...
    handle->paused   = false;
    GPTMaster__SetEnabled(handle, true);
    return ERROR_NONE;
}

int32_t GPTMaster_Start_Freerun(GPTMaster *handle, uint32_t speedHz)
{
    if (!handle || !handle->open) {
        return ERROR_PARAMETER;
    }
    ...
    // Setup Timer control registers
    if ((speedHz != 0) &&
        ((error = GPTMaster__SetSpeed(handle, speedHz)) != ERROR_NONE)) {
        return error;
    }
...
int32_t GPTMaster_WaitTimer_Blocking(
    GPTMaster *handle,
    uint32_t   speedHz,
    uint32_t   timeout,
    GPT_Units  units)
{
    ...
    uint32_t startCount = GPTMaster_GetCount(handle);

    // Start timeout and wait for it, don't re-set the speed
    if ((error = GPTMaster_Start_Freerun(handle, 0)) != ERROR_NONE) {
        GPTMaster_Stop(handle);
        return error;
    }
    uint32_t count;
    ...
    case MT3620_UNIT_GPT1:
        ...
        while (((count = GPTMaster_GetCount(handle)) > (MT3620_GPT_MAX_COUNT - nCount)) ||
            (count == startCount)) { }
        break;
    ...
    case MT3620_UNIT_GPT4:
        ...
        while (GPTMaster_GetCount(handle) < nCount) {}
        break;
    ...
    return GPTMaster_Stop(handle);
}

static void GPTMaster_IRQ(GPTMaster *handle)
{
    if (!handle || !handle->open) {
        return;
    }
...
void gpt_int_b(void)
{
    ...
    if (gpt0_int) {
        GPTMaster_IRQ(&context[0]);
    }
    if (gpt1_int) {
        GPTMaster_IRQ(&context[1]);
    }
}
...
{
    GPTMaster *handle = &context[3];
    GPTMaster_IRQ(handle);
    ...
    GPT_TestSpeeds *testSpeeds)
{
    ...
    case MT3620_UNIT_GPT4:
        // 2 speed timer
        testSpeeds->speeds[0] = MT3620_GPT_4_LOW_SPEED;
        testSpeeds->speeds[1] = MT3620_GPT_4_HIGH_SPEED;
        testSpeeds->count = 2;
        break;
