...
static inline int32_t UART__PrintSign(UART *handle, int32_t *value)
{
    int32_t error;
    if (*value < 0) {
    ...
    char      filler)
{
    ...
    int32_t error;
    if ((error = UART__PrintSign(handle, (int32_t*)(&value))) != ERROR_NONE) {
        return error;
    }
...
int32_t UART_Printf(UART *handle, ...)
{
    va_list args;
