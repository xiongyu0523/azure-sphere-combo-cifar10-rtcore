...

{
    ...
    NVIC_EnableIRQ(MT3620_DMA_INTERRUPT, ADC_PRIORITY);
...
void ADC_Close(AdcContext *handle)
{
    ...
    NVIC_DisableIRQ(MT3620_DMA_INTERRUPT);
    ...
    uint16_t referenceVoltage)
{
    ...
    GPTMaster *timer = GPTMaster_Open(MT3620_UNIT_GPT3);
    GPTMaster_WaitTimer_Blocking(timer, MT3620_GPT_3_LOW_SPEED, 50, GPT_UNITS_MICROSEC);
    GPTMaster_Close(timer);
    ...
    //Reset the DMA fifo before enabling
    mt3620_dma[MT3620_ADC_DMA_CHANNEL].swptr = mt3620_dma[MT3620_ADC_DMA_CHANNEL].hwptr;

    //Set DMA registers
    mt3620_dma_global->ch_en_set = (1 << 29);
    ...
    /* TODO: Come up with a sane default value for this                       */
    mt3620_dma[MT3620_ADC_DMA_CHANNEL].altlen = 0;
    MT3620_DMA_FIELD_WRITE(MT3620_ADC_DMA_CHANNEL, count, len, handle->fifoSize - 1);
    mt3620_dma[MT3620_ADC_DMA_CHANNEL].to = 0;
    ...
    MT3620_DMA_FIELD_WRITE(MT3620_ADC_DMA_CHANNEL, ackint, toack, 1);
    ...
    MT3620_DMA_FIELD_WRITE(MT3620_ADC_DMA_CHANNEL, start, str, 1);
...
void m4dma_irq_b(void)
{
    mt3620_dma_ffsta_t fifoStatus = { .mask = mt3620_dma[MT3620_ADC_DMA_CHANNEL].ffsta };

    AdcContext *handle = &context[0];

    mt3620_dma_intsta_t interruptStatus = { .mask = mt3620_dma[MT3620_ADC_DMA_CHANNEL].intsta };

    if (interruptStatus._int && fifoStatus.full) {
        // Fallthrough
    } else if (interruptStatus.toint) {
        return;
    }

    mt3620_dma_swptr_t swptr = { .mask = mt3620_dma[MT3620_ADC_DMA_CHANNEL].swptr };
    mt3620_dma_hwptr_t hwptr = { .mask = mt3620_dma[MT3620_ADC_DMA_CHANNEL].hwptr };

    unsigned count;
    if (swptr.swptr_wrap != hwptr.hwptr_wrap) {
        count = (handle->fifoSize - swptr.swptr) + hwptr.hwptr;
    } else {
        count = (hwptr.hwptr - swptr.swptr);
    }

    int j = 0;
    for (j = 0; j < count; j++) {
        // FIFO size must be power of two.
        unsigned entry = (swptr.swptr + j) & (handle->fifoSize - 1);

        handle->data[j].value   = (handle->rawData[entry] >> 4) & 0xFFF;
        handle->data[j].channel = handle->rawData[entry] & 0xF;
    }
    mt3620_dma[MT3620_ADC_DMA_CHANNEL].swptr = hwptr.mask;
    MT3620_DMA_FIELD_WRITE(MT3620_ADC_DMA_CHANNEL, ackint, ack, 1);

    handle->callback(ADC_DATA_READY);
}
